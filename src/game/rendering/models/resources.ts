...
export function buildResourceModel(cache: ResourceCache, type: HarvestableResourceType, id: string): ResourceModel {
  ...
  const bed = new THREE.Mesh(
    cache.geometry(`${type}-bed`, () => new THREE.CylinderGeometry(isMatter ? 2 : 1.7, isMatter ? 2.3 : 2, 0.24, isMatter ? 7 : 6)),
    cache.standard(`${type}-bed-mat`, {
      color: isMatter ? 0x6d5b41 : 0x2c4f4d,
      roughness: 0.95,
      metalness: 0.05,
    }),
  );
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const radius = isMatter ? 0.95 : 0.8;
    mesh.position.set(Math.cos(angle) * radius, isMatter ? 0.5 : 1.05, Math.sin(angle) * radius);
  ...
  if (!isMatter) {
    const halo = new THREE.Mesh(
      cache.geometry('energy-halo', () => new THREE.TorusGeometry(1.15, 0.05, 6, 20)),
      cache.standard('energy-halo-mat', { color: 0x7ff2e2, emissive: 0x35d8c0, emissiveIntensity: 1.6, roughness: 0.3 }),
    );
