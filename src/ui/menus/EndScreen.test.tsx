...
describe('end screen', () => {
  it('stays hidden until a result is reported, then shows the outcome and statistics', () => {
    ...
    expect(renderToStaticMarkup(<EndScreenCard result={store.matchResult} summary={store.matchSummary} onRestart={store.restartMatch} />)).toBe('');
    ...
    const html = renderToStaticMarkup(<EndScreenCard result={victorious.matchResult} summary={victorious.matchSummary} onRestart={victorious.restartMatch} />);
  ...
  it('renders Defeat and clears the result when a new match is requested', () => {
    ...
    expect(renderToStaticMarkup(<EndScreenCard result={defeated.matchResult} summary={defeated.matchSummary} onRestart={defeated.restartMatch} />)).toContain('DEFEAT');
    ...
    expect(renderToStaticMarkup(<EndScreenCard result={restarted.matchResult} summary={restarted.matchSummary} onRestart={restarted.restartMatch} />)).toBe('');
  });
});
