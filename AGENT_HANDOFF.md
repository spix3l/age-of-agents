...
- `art/` is the reference-art pipeline (sheet → per-model crops), documented in `art/README.md`; the crops are the reference the hand-written procedural models are built against. `scripts/` holds the capture CLIs; their bulk outputs (`models/`, `models3d/`) are gitignored scratch.
